...
Users multi-select what they're growing during onboarding: cannabis, vegetable garden, houseplants. Each creates a garden in the app.
...
3. **Merger** — deduplicates against existing catalog entries (match on normalized name + breeder)
...
|-----------|----------|------------|-----------|
| Temp (C) | 21-26 | 18-24 (indica) / 20-30 (sativa) | 22-25 |
| Humidity (%) | ~80 | 50-70 | 40-50 |
| EC (mS/cm) | 0.4-0.6 | 1.2-2.0 | 1.4-2.0 |
| pH (soil) | 6.0-7.0 | 6.0-7.0 | 6.0-7.0 |
...
2. Fuzzy match against `name`, `aliases` JSON, and `subcategory`
3. Results ranked: exact > prefix > fuzzy > subcategory match
4. Each result shows: name, subcategory, breeder, difficulty, thumbnail
