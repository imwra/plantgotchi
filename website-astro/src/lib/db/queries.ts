...
export async function createPlant(plant: Omit<Plant, 'created_at' | 'updated_at'>): Promise<void> {
  const db = getDb();
  await db.execute({
    sql: `INSERT INTO plants (id, user_id, name, species, emoji, photo_url, moisture_min, moisture_max, temp_min, temp_max, light_preference, plant_type, strain_id, strain_name, strain_type, environment, current_phase, phase_started_at, grow_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [plant.id, plant.user_id, plant.name, plant.species, plant.emoji, plant.photo_url, plant.moisture_min, plant.moisture_max, plant.temp_min, plant.temp_max, plant.light_preference, plant.plant_type, plant.strain_id, plant.strain_name, plant.strain_type, plant.environment, plant.current_phase, plant.phase_started_at, plant.grow_id],
  });
}
