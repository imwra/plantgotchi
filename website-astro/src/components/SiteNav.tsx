  ...
  return (
    <nav className="sticky top-0 z-50 bg-bg/90 backdrop-blur-sm border-b-2 border-text">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        <a href={`${prefix}/`} className="pixel-font text-primary-dark text-xs sm:text-sm tracking-tight">
          Plantgotchi
        </a>
          ...
          {userName ? (
            <>
              ...
              <button
                ...
                className="pixel-font text-[8px] sm:text-[10px] bg-text-mid text-bg px-3 py-2 pixel-border hover:bg-text transition-colors"
              >
                {labels.logout}
              </button>
            </>
            ...
            <a
              href={loginHref}
              className="pixel-font text-[8px] sm:text-[10px] bg-primary text-bg px-3 py-2 pixel-border hover:bg-primary-dark transition-colors"
            >
              {labels.login}
            </a>
          )}
          ...
          <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col gap-1">
            {navLinks.map((l) => (
              <a
                ...
                className="pixel-font text-[9px] text-text-mid hover:text-primary-dark hover:bg-bg-warm px-3 py-2.5 rounded transition-colors"
              >
                {l.label}
              </a>
            ))}
            {userName ? (
              <div className="px-3 py-2.5 flex items-center justify-between">
                ...
                <button
                  ...
                  className="pixel-font text-[8px] text-danger"
                >
                  {labels.logout}
                </button>
              </div>
            ) : (
              <a href={loginHref} className="pixel-font text-[9px] text-primary-dark px-3 py-2.5">
                {labels.login}
              </a>
            )}
          </div>
