...
const translations = {
  en: {
      ...
      sections: [
        ...
        },
        {
          ...
          children: [
            ...
            { title: 'Media Library', leaves: ['Upload images, files, and media to R2 object storage', 'Presigned URLs for direct browser-to-R2 uploads', 'media_assets table tracks file metadata (type, size, MIME)', 'Reuse media across courses and modules'] },
          ],
        },
        {
          title: '16. Courses', color: C.danger,
          children: [
            { title: 'Course Structure', leaves: ['Course > Phases > Modules > Content Blocks', '6 Block Types: video, text (markdown), quiz, image, file, code'] },
            { title: 'Course Editor', leaves: ['Add/edit/reorder phases, modules, blocks', 'Inline block editors for all 6 types', 'Media picker for images and files', 'Status: draft > published > archived', 'Auto-generated slug'] },
            { title: 'Search & Discovery', leaves: ['FTS5 full-text search on course title + description', 'Tag system with course_tags join table', 'Sort by: newest, popular, alphabetical', 'Tag filter chips in catalog view'] },
            { title: 'Quiz Scoring', leaves: ['Single-choice and multi-select quizzes', 'Server-side scoring with pass threshold', 'Partial credit: (correct - wrong) / total', 'Max attempts with retry support', 'Duplicate completion guard'] },
            { title: 'Enrollment', leaves: ['Free or paid (v1: no Stripe)', 'One enrollment per user per course'] },
            { title: 'Learner View', leaves: ['Phase sidebar + content area + progress ring', 'Video player, quiz, image, file download, code block', 'Mark as complete or auto-complete on quiz pass'] },
            { title: 'Progress Tracking', leaves: ['Per-module, per-phase completion', 'Quiz answers + scores stored as JSON', 'Progress ring with percentage'] },
          ],
        },
      ],
    },
    walkthrough: [
      ...
      { title: 'Creators', color: C.danger, desc: 'Users can opt-in to become course creators. They get a profile, dashboard, and media library to manage their courses.', details: ['Become a Creator: display name + bio', 'Creator Dashboard: course list with enrollment counts', 'Profile management: update name, bio, avatar', 'Media Library: upload to R2 with presigned URLs', 'Access control: ownership verified for edit/delete'] },
      { title: 'Courses', color: C.danger, desc: 'A learning marketplace with 6 block types, FTS5 search, tag filtering, quiz scoring, and media uploads.', details: ['Course > Phases > Modules > 6 Block Types (video, text, quiz, image, file, code)', 'Course Editor: inline editors for all block types + media picker', 'FTS5 full-text search + tag system + sort options', 'Quiz scoring: single/multi-select, pass threshold, server-side grading', 'Enrollment: free or paid (v1: no Stripe)', 'Learner View: phase sidebar + content + progress ring', 'Progress: per-module completion, quiz scores + answers stored'] },
  ...
  'pt-br': {
      ...
      sections: [
        ...
        },
        {
          ...
          children: [
            ...
            { title: 'Biblioteca de Midia', leaves: ['Upload de imagens, arquivos e midia para R2 object storage', 'URLs pre-assinadas para upload direto do navegador ao R2', 'Tabela media_assets rastreia metadados (tipo, tamanho, MIME)', 'Reutilizar midia entre cursos e modulos'] },
          ],
        },
        {
          title: '16. Cursos', color: C.danger,
          children: [
            { title: 'Estrutura do Curso', leaves: ['Curso > Fases > Modulos > Blocos de Conteudo', '6 Tipos de Bloco: video, texto (markdown), quiz, imagem, arquivo, codigo'] },
            { title: 'Editor de Curso', leaves: ['Adicionar/editar/reordenar fases, modulos, blocos', 'Editores inline para todos os 6 tipos', 'Seletor de midia para imagens e arquivos', 'Status: rascunho > publicado > arquivado', 'Slug auto-gerado'] },
            { title: 'Busca & Descoberta', leaves: ['Busca full-text FTS5 em titulo + descricao do curso', 'Sistema de tags com tabela course_tags', 'Ordenar por: mais recentes, populares, alfabetico', 'Chips de filtro de tags na visao do catalogo'] },
            { title: 'Pontuacao de Quiz', leaves: ['Quizzes de escolha unica e multipla selecao', 'Pontuacao no servidor com limiar de aprovacao', 'Credito parcial: (corretas - erradas) / total', 'Maximo de tentativas com suporte a nova tentativa', 'Protecao contra conclusao duplicada'] },
            { title: 'Inscricao', leaves: ['Gratis ou pago (v1: sem Stripe)', 'Uma inscricao por usuario por curso'] },
            { title: 'Visao do Aluno', leaves: ['Barra lateral de fases + area de conteudo + anel de progresso', 'Player de video, quiz, imagem, download de arquivo, bloco de codigo', 'Marcar como concluido ou auto-completar ao passar no quiz'] },
            { title: 'Acompanhamento de Progresso', leaves: ['Conclusao por modulo, por fase', 'Respostas + pontuacoes de quiz armazenadas em JSON', 'Anel de progresso com porcentagem'] },
          ],
        },
      ],
    },
    walkthrough: [
      ...
      { title: 'Criadores', color: C.danger, desc: 'Usuarios podem optar por se tornar criadores de cursos. Recebem perfil, painel e biblioteca de midia para gerenciar cursos.', details: ['Tornar-se Criador: nome + bio', 'Painel do Criador: lista cursos com inscricoes', 'Gerenciamento de perfil: atualizar nome, bio, avatar', 'Biblioteca de Midia: upload para R2 com URLs pre-assinadas', 'Controle de acesso: propriedade verificada para editar/excluir'] },
      { title: 'Cursos', color: C.danger, desc: 'Marketplace de aprendizado com 6 tipos de bloco, busca FTS5, filtragem por tags, pontuacao de quiz e uploads de midia.', details: ['Curso > Fases > Modulos > 6 Tipos de Bloco (video, texto, quiz, imagem, arquivo, codigo)', 'Editor: editores inline para todos os tipos + seletor de midia', 'Busca full-text FTS5 + sistema de tags + opcoes de ordenacao', 'Pontuacao de quiz: unica/multipla, limiar de aprovacao, avaliacao no servidor', 'Inscricao: gratis ou pago (v1: sem Stripe)', 'Visao do Aluno: barra lateral de fases + conteudo + anel de progresso', 'Progresso: conclusao por modulo, pontuacoes + respostas armazenadas'] },
...
const NODES: FlowNode[] = [
  ...
  { id: 'add_blocks', label: 'Add Blocks', cat: 'lms', col: 3, row: 15, tooltip: '6 tipos: video, texto, quiz, imagem, arquivo, codigo / 6 types: video, text, quiz, image, file, code' },
  ...
  { id: 'course_catalog', label: 'Course Catalog', cat: 'lms', col: 1, row: 16, tooltip: 'Marketplace com busca FTS5, tags e ordenacao / FTS5 search, tags, sort' },
  ...
  { id: 'quiz_block', label: 'Quiz Block', cat: 'lms', col: 2, row: 17, tooltip: 'Quiz unica/multipla com pontuacao no servidor / Single/multi-choice with server scoring' },
  ...
  { id: 'media_library', label: 'Media Library', cat: 'lms', col: 4, row: 14, tooltip: 'Upload para R2 com URLs pre-assinadas / Upload to R2 with presigned URLs' },
  { id: 'image_block', label: 'Image Block', cat: 'lms', col: 4, row: 15, tooltip: 'Bloco de imagem com alt e legenda / Image with alt text and caption' },
  { id: 'file_block', label: 'File Block', cat: 'lms', col: 4, row: 16, tooltip: 'Download de arquivo com tamanho / File download with size display' },
  { id: 'code_block', label: 'Code Block', cat: 'lms', col: 4, row: 17, tooltip: 'Snippet de codigo com copia / Code snippet with copy button' },
  { id: 'fts5_search', label: 'FTS5 Search', cat: 'lms', col: 0, row: 18, tooltip: 'Busca full-text em titulo + descricao / Full-text search on title + description' },
  { id: 'tag_filter', label: 'Tag Filter', cat: 'lms', col: 1, row: 18, tooltip: 'Filtragem por tags com chips / Tag filtering with chips' },
];
...
const EDGES: FlowEdge[] = [
  ...
  // LMS media & blocks
  { from: 'creator_dash', to: 'media_library' }, { from: 'media_library', to: 'course_editor' },
  { from: 'add_blocks', to: 'image_block' }, { from: 'add_blocks', to: 'file_block' },
  { from: 'add_blocks', to: 'code_block' }, { from: 'media_library', to: 'image_block' },
  { from: 'learner_view', to: 'image_block' }, { from: 'learner_view', to: 'file_block' },
  { from: 'learner_view', to: 'code_block' },
  // LMS search
  { from: 'course_catalog', to: 'fts5_search' }, { from: 'course_catalog', to: 'tag_filter' },
  // LMS to web
